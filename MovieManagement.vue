  ...
  export default {
      ...
      async AddMovie() {
        try {
          ...
          const response = await axios.post(
            "http://localhost:5148/api/AdminMovie/AddMovie",
            formData,
            {
              headers: {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5148/api/AdminMovie/UpdateMovie/${movieId}`,
        formData,
        {
          headers: {
            "Content-Type": "multipart/form-data",
      ...
      async deleteMovie(index) {
        if (confirm("Bạn có chắc muốn xóa phim này?")) {
          try {
            ...
            await axios.delete(`http://localhost:5148/api/AdminMovie/de/${movieId}`);
