<template>
    ...
    methods: {
      async fetchCategories() {
        try {
          const res = await axios.get(
            "http://localhost:5148/api/AdminCategories?sortBy=CategoryId&sortDirection=asc&page=1&pageSize=100"
          );
      ...
      async addCategory() {
        ...
        try {
          ...
          await axios.post(
            `http://localhost:5148/api/AdminCategories?categoryName=${encodeURIComponent(this.categoryForm.categoryName)}`
          );
      ...
      async updateCategory() {
        ...
        try {
          ...
          await axios.put(
            `http://localhost:5148/api/AdminCategories/${this.categoryForm.categoryId}?categoryName=${encodeURIComponent(this.categoryForm.categoryName)}`
          );
          await this.fetchCategories();
          this.resetForm();
  ...
  try {
    const res = await axios.delete(
      `http://localhost:5148/api/AdminCategories/${id}`,
