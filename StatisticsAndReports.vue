                      ...
                      <tbody>
                        <tr v-for="episode in series.episodes" :key="episode.episodeId">
                          ...
                          <td>
                            ...
                            <button @click="deleteEpisode(episode.episodeId, series.seriesId)" class="delete-button">Xóa</button>
                          </td>
                        </tr>
                      </tbody>
                    </table>
...
export default {
  ...
  openEditEpisodeModal(episode) {
    // Mở modal sửa tập phim
    console.log("Sửa tập phim:", episode);
    // Gọi modal hoặc component sửa
  },
  ...
  deleteEpisode(episodeId, seriesId) {
    if (confirm("Bạn có chắc muốn xóa tập phim này không?")) {
      axios.delete(`/api/episodes/${episodeId}`)
        .then(() => {
          const series = this.paginatedSeries.find(s => s.seriesId === seriesId);
          if (series) {
...
.modal-content .buttons-container {
  display: flex;
  justify-content: space-between; /* Đảm bảo nút cách nhau đều */
...
.modal-content button {
  ...
  width: 48%; /* Mỗi nút chiếm 48% chiều rộng của modal */
}
