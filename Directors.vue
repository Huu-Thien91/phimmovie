    ...
    <div class="container">
      ...
      <div v-if="showForm" class="form">
        ...
        <div class="form-grid">
          ...
          <input v-model="directorForm.nationality" type="text" placeholder="Quốc tịch" class="input" />
          <input v-model="directorForm.professional" type="text" placeholder="Chuyên môn" class="input" />
          <!-- Input file để chọn ảnh đại diện -->
          <input type="file" @change="handleFileUpload" class="input" />
          <textarea v-model="directorForm.description" placeholder="Mô tả" class="input full"></textarea>
        </div>
        <div class="form-actions">
          <button @click="editingDirector ? updateDirector() : addDirector()" class="btn btn-blue">
            {{ editingDirector ? 'Cập nhật' : 'Thêm' }}
          </button>
          <button v-if="editingDirector" @click="resetForm" class="btn btn-gray">Hủy</button>
        </div>
      </div>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-for="director in paginatedDirectors" :key="director.directorID">
            ...
            <td>
              <img :src="director.avatarUrl" alt="avatar" class="avatar" />
            </td>
            <td>
    ...
    methods: {
      async fetchDirectors() {
        try {
          const res = await axios.get(
            "http://localhost:5148/api/AdminDirectors/List-Directors?sortBy=DirectorId&sortDirection=asc&page=1&pageSize=100"
          );
      ...
      async addDirector() {
        try {
          ...
          formData.append("Description", this.directorForm.description);
          formData.append("Nationality", this.directorForm.nationality);
          formData.append("Professional", this.directorForm.professional);
          if (this.avatarFile) {
            formData.append("AvatarUrlFile", this.avatarFile);
          }
          await axios.post("http://localhost:5148/api/AdminDirectors/AddDirector", formData);
      ...
      async updateDirector() {
        try {
          ...
          await axios.put(
            `http://localhost:5148/api/AdminDirectors/UpdateDirector/${this.directorForm.directorID}`,
            formData
          );
          await this.fetchDirectors();
          this.resetForm();
      ...
      async deleteDirector(id) {
        if (confirm("Bạn có chắc chắn muốn xóa?")) {
          try {
            await axios.delete(`http://localhost:5148/api/AdminDirectors/DeleteDirector/${id}`);
            await this.fetchDirectors();
